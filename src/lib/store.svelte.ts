...
let templateIsPublic = $state(false);
...
const themeColor = $derived(
  blocks.find((b) => b.type === "header")?.props.accentColor ?? "#1daa82",
);
...
function canMove(id: string, direction: "up" | "down"): boolean {
  ...
  if (blocks[target].type === "header" || blocks[target].type === "footer")
    return false;
  return true;
}
...
function setTemplate(id: string, name: string, newBlocks: Block[], isPublic = false) {
  templateId = id;
  templateName = name;
  setBlocks(newBlocks);
      templateIsPublic = isPublic; 
...
export const editor = {
  ...
  set templateCategory(v: string) {
    templateCategory = v;
    dirty = true;
  },
  get templateIsPublic() {
    return templateIsPublic;
  },
  set templateIsPublic(v: boolean) {
    templateIsPublic = v;
    dirty = true;
  },
  get templateId() {
